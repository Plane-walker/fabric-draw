...
def generate_order_msp(order_id, order_information, crypto_base='/root/opt'):
    ...
    ftp_client.put(file_name, f'{crypto_base}/{file_name}')
    ssh.exec_command(f'python node_build.py --func_name org_msp_generate "{crypto_base}" "{group_name}" "{domain}" "{node_port}"')
    ssh.exec_command(f'python node_build.py --func_name peer_msp_generate "{crypto_base}" "{node_name}" "{group_name}" "{domain}" "{node_port}"')
